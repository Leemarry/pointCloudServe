...
public class AESUtil {
    ...
    // 加密
    public static String encryptAES(String message, String secretKeyStr, String iv) {
        try {
            return encryptAES(message.getBytes(), secretKeyStr, iv);
        } catch (Exception e) {
            return message;
        }
    }
    ...
    public static String encryptAES(byte[] content, String secretKeyStr, String iv) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException, InvalidAlgorithmParameterException {
        ...
        // 执行加密并返回经 BASE64 处理之后的密文
        return Base64.getEncoder().encodeToString(cipher.doFinal(content));
    }


    // 解密
    public static String decryptAES(String message, String secretKeyStr, String iv) {
        try {
            return decryptAES(message.getBytes(), secretKeyStr, iv);
        } catch (Exception e) {
            return message;
        }
    }
