...
public class CrcUtil {
    public static boolean CheckCrc16Packet(byte[] bytes) {
        if (bytes.length < 3) return false;
        byte[] crc = Crc16Packet(bytes, bytes.length - 2);
        if (bytes[bytes.length - 2] == crc[0] && bytes[bytes.length - 1] == crc[1])
            return true;
        return false;
    }
    ...
    public static byte[] getCRC3(byte[] data, int length) {
        if (data.length < length) return new byte[2];
        int crc = 0x0000ffff;
        int ucCRCHi = 0x00ff;
        int ucCRCLo = 0x00ff;
        int iIndex;
...
//        return String.format("%04X", crc);
        ...
        crcbytes[0] = (byte) (crc & 0x000000FF);
        crcbytes[1] = (byte) ((crc & 0x0000FF00) >> 8);
        return crcbytes;
    }

    public static byte[] Crc16Packet(byte[] bytes, int length) {
...
//        8) 最后得到的 CRC寄存器内容即为 CRC码。
        ...
        if (bytes.length < length) return new byte[2];
        int CRC = 0x0000ffff;
